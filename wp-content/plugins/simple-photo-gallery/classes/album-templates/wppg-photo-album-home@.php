...
<?php
	$intg = 0;
        ...
        if ($wpdb->num_rows > 0 && $items != NULL)
        {
            ...
            {
		?>
		<div class="wppg_album_item_container alb_<?php echo $intg; ?>" id="alb_<?php echo $intg; ?>"><?php //echo $thumb_image_url."---".$al_thumb_url; ?>
		<?php
                $p = get_post($item->page_id);
                $page_link = get_permalink($p->ID);
                $album_thumb_url=$item->thumbnail_url;
		$al_thumb_url=$item->thumbnail_url;
		$size_img = getimagesize($al_thumb_url);
		$img_w = $size_img[0];
		$img_h = $size_img[1];
		//echo "<pre>";
		//print_r($size_img);
                ...
                ?> 
                    ...
                    <div class="wppg_album_item_top">
                        ...
                        <a href='<?php echo $page_link ?>'>
                                <img class="wppg_album_item_thumb" src='<?php echo $al_thumb_url ?>' alt='<?php echo $album_name; ?>' title="<?php echo $album_name; ?>" />
                        </a>
                        </div>
                    </div>
                    <div class="wppg_album_item_bottom">
                        <a href="<?php echo $page_link; ?>" title="<?php echo $album_name; ?>"><div class="wppg_album_item_name"><p style="top: 50%; position: relative; color: #fff;"><?php echo $album_name_short; ?></p></div></a>
                    </div>
